v {xschem version=3.4.6 file_version=1.2}
G {}
K {}
V {}
S {}
E {}
T {dummy devices} 80 -2050 0 0 0.3 0.3 {}
T {dummy devices} 290 60 0 0 0.3 0.3 {}
N 270 -100 270 -50 {lab=bot}
N 130 -130 250 -130 { lab=vss}
N 130 -180 130 -160 { lab=#net1}
N 130 -180 270 -180 { lab=#net1}
N 130 -100 130 -70 { lab=bot}
N 20 -130 90 -130 { lab=ctrl[0]}
N 270 -180 270 -160 { lab=#net1}
N 130 -250 250 -250 { lab=vss}
N 130 -300 130 -280 { lab=#net2}
N 130 -300 270 -300 { lab=#net2}
N 20 -250 90 -250 { lab=ctrl[1]}
N 270 -220 270 -180 { lab=#net1}
N 270 -300 270 -280 { lab=#net2}
N 130 -220 130 -200 { lab=bot}
N 130 -370 250 -370 { lab=vss}
N 130 -420 130 -400 { lab=#net3}
N 130 -420 270 -420 { lab=#net3}
N 20 -370 90 -370 { lab=ctrl[2]}
N 270 -340 270 -300 { lab=#net2}
N 270 -420 270 -400 { lab=#net3}
N 130 -340 130 -320 { lab=bot}
N 130 -490 250 -490 { lab=vss}
N 130 -540 130 -520 { lab=#net4}
N 130 -540 270 -540 { lab=#net4}
N 20 -490 90 -490 { lab=ctrl[3]}
N 270 -460 270 -420 { lab=#net3}
N 270 -540 270 -520 { lab=#net4}
N 130 -460 130 -440 { lab=bot}
N 130 -610 250 -610 { lab=vss}
N 130 -660 130 -640 { lab=#net5}
N 130 -660 270 -660 { lab=#net5}
N 20 -610 90 -610 { lab=ctrl[4]}
N 270 -580 270 -540 { lab=#net4}
N 270 -660 270 -640 { lab=#net5}
N 130 -580 130 -560 { lab=bot}
N 130 -730 250 -730 { lab=vss}
N 130 -780 130 -760 { lab=#net6}
N 130 -780 270 -780 { lab=#net6}
N 20 -730 90 -730 { lab=ctrl[5]}
N 270 -700 270 -660 { lab=#net5}
N 270 -780 270 -760 { lab=#net6}
N 130 -700 130 -680 { lab=bot}
N 130 -850 250 -850 { lab=vss}
N 130 -900 130 -880 { lab=#net7}
N 130 -900 270 -900 { lab=#net7}
N 20 -850 90 -850 { lab=ctrl[6]}
N 270 -820 270 -780 { lab=#net6}
N 270 -900 270 -880 { lab=#net7}
N 130 -820 130 -800 { lab=bot}
N 130 -970 250 -970 { lab=vss}
N 130 -1020 130 -1000 { lab=#net8}
N 130 -1020 270 -1020 { lab=#net8}
N 20 -970 90 -970 { lab=ctrl[7]}
N 270 -940 270 -900 { lab=#net7}
N 270 -1020 270 -1000 { lab=#net8}
N 130 -940 130 -920 { lab=bot}
N 130 -1570 250 -1570 { lab=vss}
N 130 -1620 130 -1600 { lab=#net9}
N 130 -1620 270 -1620 { lab=#net9}
N 20 -1570 90 -1570 { lab=ctrl[12]}
N 270 -1540 270 -1500 { lab=#net10}
N 270 -1620 270 -1600 { lab=#net9}
N 130 -1540 130 -1520 { lab=bot}
N 130 -1690 250 -1690 { lab=vss}
N 130 -1740 130 -1720 { lab=#net11}
N 130 -1740 270 -1740 { lab=#net11}
N 20 -1690 90 -1690 { lab=ctrl[13]}
N 270 -1660 270 -1620 { lab=#net9}
N 270 -1740 270 -1720 { lab=#net11}
N 130 -1660 130 -1640 { lab=bot}
N 130 -1810 250 -1810 { lab=vss}
N 130 -1860 130 -1840 { lab=#net12}
N 130 -1860 270 -1860 { lab=#net12}
N 20 -1810 90 -1810 { lab=ctrl[14]}
N 270 -1780 270 -1740 { lab=#net11}
N 270 -1860 270 -1840 { lab=#net12}
N 130 -1780 130 -1760 { lab=bot}
N 130 -1930 250 -1930 { lab=vss}
N 130 -1980 130 -1960 { lab=top}
N 130 -1980 270 -1980 { lab=top}
N 20 -1930 90 -1930 { lab=ctrl[15]}
N 270 -1900 270 -1860 { lab=#net12}
N 270 -1980 270 -1960 { lab=top}
N 130 -1900 130 -1880 { lab=bot}
N 130 -1090 250 -1090 { lab=vss}
N 130 -1140 130 -1120 { lab=#net13}
N 130 -1140 270 -1140 { lab=#net13}
N 20 -1090 90 -1090 { lab=ctrl[8]}
N 270 -1060 270 -1020 { lab=#net8}
N 270 -1140 270 -1120 { lab=#net13}
N 130 -1060 130 -1040 { lab=bot}
N 130 -1210 250 -1210 { lab=vss}
N 130 -1260 130 -1240 { lab=#net14}
N 130 -1260 270 -1260 { lab=#net14}
N 20 -1210 90 -1210 { lab=ctrl[9]}
N 270 -1180 270 -1140 { lab=#net13}
N 270 -1260 270 -1240 { lab=#net14}
N 130 -1180 130 -1160 { lab=bot}
N 130 -1330 250 -1330 { lab=vss}
N 130 -1380 130 -1360 { lab=#net15}
N 130 -1380 270 -1380 { lab=#net15}
N 20 -1330 90 -1330 { lab=ctrl[10]}
N 270 -1300 270 -1260 { lab=#net14}
N 270 -1380 270 -1360 { lab=#net15}
N 130 -1300 130 -1280 { lab=bot}
N 130 -1450 250 -1450 { lab=vss}
N 130 -1500 130 -1480 { lab=#net10}
N 130 -1500 270 -1500 { lab=#net10}
N 20 -1450 90 -1450 { lab=ctrl[11]}
N 270 -1420 270 -1380 { lab=#net15}
N 270 -1500 270 -1480 { lab=#net10}
N 130 -1420 130 -1400 { lab=bot}
N 270 -2000 270 -1980 { lab=top}
N 130 -2120 220 -2120 {lab=vss}
N 220 -2120 220 -2070 {lab=vss}
N 130 -2070 220 -2070 {lab=vss}
N 40 -2120 40 -2070 {lab=vss}
N 40 -2150 130 -2150 {lab=vss}
N 40 -2120 90 -2120 {lab=vss}
N 40 -2150 40 -2120 {lab=vss}
N 130 -2090 130 -2070 {lab=vss}
N 40 -2070 130 -2070 {lab=vss}
N 380 50 430 50 {lab=vss}
N 380 -10 380 50 {lab=vss}
N 380 -10 430 -10 {lab=vss}
N 380 100 430 100 {lab=#net16}
N 380 100 380 160 {lab=#net16}
N 380 160 430 160 {lab=#net16}
C {devices/iopin.sym} 100 -10 0 1 {name=p2 lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -130 0 0 {name=M0
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -130 0 1 {name=l14 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 20 -130 0 1 {name=l16 sig_type=std_logic lab=ctrl[0]
}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -250 0 0 {name=M1
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -250 0 1 {name=l18 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -370 0 0 {name=M2
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -370 0 1 {name=l17 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -490 0 0 {name=M3
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -490 0 1 {name=l21 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -610 0 0 {name=M4
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -610 0 1 {name=l26 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -730 0 0 {name=M5
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -730 0 1 {name=l28 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -850 0 0 {name=M6
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -850 0 1 {name=l30 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -970 0 0 {name=M7
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -970 0 1 {name=l32 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1570 0 0 {name=M12
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1570 0 1 {name=l34 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1690 0 0 {name=M13
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1690 0 1 {name=l36 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1810 0 0 {name=M14
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1810 0 1 {name=l38 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1930 0 0 {name=M15
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1930 0 1 {name=l40 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1090 0 0 {name=M8
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1090 0 1 {name=l24 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1210 0 0 {name=M9
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1210 0 1 {name=l42 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1330 0 0 {name=M10
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1330 0 1 {name=l44 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -1450 0 0 {name=M11
l=0.5u
w=2u
ng=1 
m=1
}
C {devices/lab_wire.sym} 130 -1450 0 1 {name=l46 sig_type=std_logic lab=vss}
C {devices/lab_wire.sym} 130 -1880 0 0 {name=l62 sig_type=std_logic lab=bot
}
C {devices/ipin.sym} 100 -40 0 0 {name=p1 lab=ctrl[15:0]
}
C {devices/iopin.sym} 270 -50 3 1 {name=p3 lab=bot
}
C {devices/iopin.sym} 270 -2000 1 1 {name=p4 lab=top
}
C {devices/lab_wire.sym} 20 -250 0 1 {name=l1 sig_type=std_logic lab=ctrl[1]
}
C {devices/lab_wire.sym} 20 -370 0 1 {name=l2 sig_type=std_logic lab=ctrl[2]
}
C {devices/lab_wire.sym} 20 -490 0 1 {name=l3 sig_type=std_logic lab=ctrl[3]
}
C {devices/lab_wire.sym} 20 -610 0 1 {name=l4 sig_type=std_logic lab=ctrl[4]
}
C {devices/lab_wire.sym} 20 -730 0 1 {name=l5 sig_type=std_logic lab=ctrl[5]
}
C {devices/lab_wire.sym} 20 -850 0 1 {name=l6 sig_type=std_logic lab=ctrl[6]
}
C {devices/lab_wire.sym} 20 -970 0 1 {name=l7 sig_type=std_logic lab=ctrl[7]
}
C {devices/lab_wire.sym} 20 -1090 0 1 {name=l8 sig_type=std_logic lab=ctrl[8]
}
C {devices/lab_wire.sym} 20 -1210 0 1 {name=l9 sig_type=std_logic lab=ctrl[9]
}
C {devices/lab_wire.sym} 20 -1330 0 1 {name=l10 sig_type=std_logic lab=ctrl[10]
}
C {devices/lab_wire.sym} 20 -1450 0 1 {name=l11 sig_type=std_logic lab=ctrl[11]
}
C {devices/lab_wire.sym} 20 -1570 0 1 {name=l12 sig_type=std_logic lab=ctrl[12]
}
C {devices/lab_wire.sym} 20 -1690 0 1 {name=l13 sig_type=std_logic lab=ctrl[13]
}
C {devices/lab_wire.sym} 20 -1810 0 1 {name=l15 sig_type=std_logic lab=ctrl[14]
}
C {devices/lab_wire.sym} 20 -1930 0 1 {name=l19 sig_type=std_logic lab=ctrl[15]
}
C {devices/lab_wire.sym} 130 -1760 0 0 {name=l20 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1640 0 0 {name=l22 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1520 0 0 {name=l23 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1400 0 0 {name=l25 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1280 0 0 {name=l27 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1160 0 0 {name=l29 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -1040 0 0 {name=l31 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -920 0 0 {name=l33 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -800 0 0 {name=l35 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -680 0 0 {name=l37 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -560 0 0 {name=l39 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -440 0 0 {name=l41 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -320 0 0 {name=l43 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -200 0 0 {name=l45 sig_type=std_logic lab=bot
}
C {devices/lab_wire.sym} 130 -70 0 0 {name=l47 sig_type=std_logic lab=bot
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1930 0 0 {name=R15
l=3.4u
w=1.41u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -250 0 0 {name=R1
l=3.4u
w=1.41u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -130 0 0 {name=R16
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -370 0 0 {name=R2
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -490 0 0 {name=R3
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -610 0 0 {name=R4
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -730 0 0 {name=R5
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -850 0 0 {name=R6
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -970 0 0 {name=R7
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1090 0 0 {name=R8
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1210 0 0 {name=R9
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1330 0 0 {name=R10
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1450 0 0 {name=R11
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1570 0 0 {name=R12
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1690 0 0 {name=R13
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 270 -1810 0 0 {name=R14
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/sg13_hv_nmos.sym} 110 -2120 0 0 {name=M16
l=0.5u
w=2u
ng=1 
m=2
}
C {lab_pin.sym} 220 -2120 0 1 {name=p5 sig_type=std_logic lab=vss}
C {sg13cmos5l_pr/rhigh.sym} 430 20 0 0 {name=R17
w=1.41u
l=3.4u
body=vss
m=1
}
C {sg13cmos5l_pr/rhigh.sym} 430 130 0 0 {name=R18
w=1.41u
l=3.4u
body=vss
m=1
}
C {lab_pin.sym} 380 50 0 0 {name=p6 sig_type=std_logic lab=vss}
C {lab_pin.sym} 380 160 0 0 {name=p7 sig_type=std_logic lab=top}
